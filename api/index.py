...
def main_page():
    ...
    return render_template('.output/server/pagesmain_page.html', is_mobile=is_mobile, title=f'Главная', friends=friends);
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and user.is_blocked:
            return render_template('.output/server/pageslogin.html', is_mobile=is_mobile,
    ...
    return render_template('.output/server/pageslogin.html', is_mobile=is_mobile, title='Авторизация', form=form)
...
def register():
    ...
    if form.validate_on_submit():
        if form.password.data != form.password2.data:
            return render_template('.output/server/pagesregister.html', is_mobile=is_mobile, title='Регистрация',
                                   form=form,
    ...
    return render_template('.output/server/pagesregister.html', is_mobile=is_mobile, title='Регистрация', form=form)
...
def profile(id):
    ...
    return render_template('.output/server/pagesprofile.html', is_mobile=is_mobile, title=f'Пользователь {id}', user=user, friends=friends, is_our_request=our_request is not None, is_his_request=his_request is not None, request=his_request);
...
def friendship(id):
    ...
    if form.validate_on_submit():
        user = db_sess.query(User).filter(User.id == id).first()
        if user is None:
            return render_template('.output/server/pagesfriendship_request.html', is_mobile=is_mobile, title='Запрос на дружбу',
                                   form=form,
    ...
    return render_template('.output/server/pagesfriendship_request.html', is_mobile=is_mobile, title='Запрос на дружбу', form=form)
...
def my_requests():
    ...
    return render_template('.output/server/pagesmy_requests.html', is_mobile=is_mobile, title='Запросы на вашу дружбу', requests=requests)
...
def chat(id):
    ...
    for msg in messages:
        if msg.attached_file is not None:
            ...
            if file.name.split('.')[-1] in ['png', 'jpg', 'bmp', 'gif', 'ico']:
                if not os.access(f'.output/staticfiles/{file.name}', os.F_OK):
                    with open(f'.output/staticfiles/{file.name}', 'wb') as f:
    ...
    return render_template('.output/server/pageschat.html', is_mobile=is_mobile, title=other.username, messages=messages, other=other, message='', images=images, files=files, none=None)
...
def edit_profile():
    ...
    return render_template('.output/server/pagesedit_profile.html', is_mobile=is_mobile, title=f'Редактировать профиль', form=form);
...
def search():
    ...
    return render_template('.output/server/pagessearch.html', is_mobile=is_mobile, title='Поиск', users=users, apolog=show_apologizion)
    ...
    if request.method == "POST":
        ...
        if request.form.get('save_button') is not None:
            if os.path.isdir(directory):
                ...
                return render_template('.output/server/pagesload_file.html', is_mobile=is_mobile, file=file, message='This path doesn\'t exist')
            file.path = directory
            db_sess.merge(file)
            db_sess.commit()
            return render_template('.output/server/pagesload_file.html', is_mobile=is_mobile, file=file, message='File was saved successfully')
    
    return render_template('.output/server/pagesload_file.html', is_mobile=is_mobile, file=file)
...
def my_chats():
    ...
    return render_template('.output/server/pagesmy_chats.html', is_mobile=is_mobile, chats=chats, others=others, last_sender=last_sender, last_msg=last_msg, last_files=last_files, unread_msgs=unread_msgs, last_times=last_times)
